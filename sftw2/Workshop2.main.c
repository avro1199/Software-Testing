...
int main(void)
{
	...
	// new code written for testing purpose by Rj Avro//
	// find string test
	char list[][MAX_STRING_LEN + 1] = {"abcdefghijklmn", "abdc", "", ""};
	int idx = findString("abdc", list, 4);
	printf("%d\n", idx);

	// add2Cart test
	struct Cart cart = {{0}, 0};
	int val = add2Cart(&cart, 5);
	printf("%d\n", val);
	...
	if (val == 0 && idx == 1 && cart.items[0] == 5 && cart.nItems == 1)
	{
		// Test passed
