...
int isTruckOverloaded(struct Truck truck, struct Shipment ship) {
    ...
    if (truck.current_weight <= 2500)
    {
        truck.current_weight += ship.weight;
...
int isBoxSizeExceeded(struct Truck truck, int boxSize) {
    ...
    if (truck.current_volume <= 100)
    {
        truck.current_volume += boxSize;
...
int validCargo(int boxSize) {
    ...
    return (boxSize == minSize || boxSize == halfSize || boxSize == maxSize);
}
